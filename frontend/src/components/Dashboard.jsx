...
const mockUser = {
  ...
  recentTransactions: [
    { id: 1, description: "Zomato Payment", amount: -345.67, date: "2025-05-21" },
    { id: 2, description: "UPI Received - CitiChain", amount: 200000, date: "2025-05-20" },
    { id: 3, description: "Electricity Bill", amount: -1020.5, date: "2025-05-18" },
    { id: 4, description: "Spotify Subscription", amount: -199, date: "2025-05-17" },
    { id: 5, description: "Freelance Credit - Code Studio", amount: 18000, date: "2025-05-16" },
    { id: 6, description: "Amazon Order #5493", amount: -1999.0, date: "2025-05-15" },
    { id: 7, description: "Phone Recharge - Airtel", amount: -349.0, date: "2025-05-14" },
    { id: 8, description: "Interest Received", amount: 450.0, date: "2025-05-13" },
    { id: 9, description: "CitiChain Rewards", amount: 750.0, date: "2025-05-12" },
...
const pieData = [
  { name: "Paid", value: mockUser.emis.filter(e => e.status === "Paid").length },
  { name: "Pending", value: mockUser.emis.filter(e => e.status === "Pending").length },
];
...
export default function Dashboard() {
  ...
  return (
    <div className="flex bg-white min-h-screen text-gray-900 font-sans relative">
      {/* Sidebar */}
      <aside className={`transition-all duration-300 ${menuOpen ? 'w-72' : 'w-20'} bg-[#012B52] text-white p-6 shadow-xl border-r border-indigo-500/10 flex flex-col`}>
        <div className="flex justify-between items-center mb-8">
          {menuOpen && (
            <div>
              <h2 className="text-xl font-bold">Welcome, {mockUser.name}</h2>
              <p className="text-sm text-indigo-200">A/C: {mockUser.accountNumber}</p>
...
<button onClick={() => setMenuOpen(!menuOpen)} className="text-white focus:outline-none">
  {menuOpen ? <ChevronLeftIcon className="w-5 h-5" /> : <ChevronRightIcon className="w-5 h-5" />}
        ...
        <nav className="space-y-4">
          ...
          <Link to="/kyc/start" className="flex items-center gap-3 px-4 py-2 hover:bg-white/10 rounded-xl"><DocumentCheckIcon className="w-5 h-5" />{menuOpen && "KYC"}</Link>
          <Link to="/rupeex/history" className="flex items-center gap-3 px-4 py-2 hover:bg-white/10 rounded-xl"><DocumentDuplicateIcon className="w-5 h-5" />{menuOpen && "Statements"}</Link>
      ...
      <main className="flex-1 p-10 space-y-10 bg-white">
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white border border-indigo-200 rounded-2xl p-6 shadow">
            ...
            <h2 className="text-2xl font-bold">₹ {mockUser.balanceINR.toLocaleString()}</h2>
          </div>
          <div className="bg-white border border-indigo-200 rounded-2xl p-6 shadow">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow">
            <h2 className="text-xl font-semibold mb-4 text-gray-800">Transaction Overview</h2>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow">
            <h2 className="text-xl font-semibold mb-4 text-gray-800">EMI Status</h2>
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
          ...
          <div className="mt-6">
            ...
            <ul className="space-y-2 text-sm max-h-60 overflow-y-auto pr-1">
              {mockUser.rupeeXHistory.map((item) => (
                <li key={item.id} className="flex justify-between border-b border-white/10 pb-1">
                  <span>{item.type} {item.to || item.from || ""} ({item.date})</span>
                  <span className={item.amount < 0 ? "text-red-300" : "text-green-300"}>
